...
InitData read_in_parameters(std::string input_file) {
    ...
    //jet-medium-response
    int temp_jet_medium_response_flag = 0;
    tempinput = Util::StringFind4(input_file, "jet_medium_response_flag");
    if (tempinput != "empty")
        istringstream(tempinput) >> temp_jet_medium_response_flag;
    parameter_list.jet_medium_response_flag = temp_jet_medium_response_flag;

    int temp_JetSourceEveryNTimestep = 0;
    tempinput = Util::StringFind4(input_file, "JetSourceEveryNTimestep");
    if (tempinput != "empty")
        istringstream(tempinput) >> temp_JetSourceEveryNTimestep;
    parameter_list.JetSourceEveryNTimestep = temp_JetSourceEveryNTimestep;

    string temp_jetName = "jet.dat";
    tempinput = Util::StringFind4(input_file, "jet_filename");
    if (tempinput != "empty")
        temp_jetName.assign(tempinput);
    parameter_list.jetName.assign(temp_jetName);

    //test single parton or not
    bool temp_single_parton = false;
    tempinput = Util::StringFind4(input_file, "single_parton");
    if (tempinput != "empty")
        istringstream(tempinput) >> temp_single_parton;
    parameter_list.single_parton = temp_single_parton;

    //one hard collision or not
    bool temp_one_hard_collision = false;
    tempinput = Util::StringFind4(input_file, "one_hard_collision");
    if (tempinput != "empty")
        istringstream(tempinput) >> temp_one_hard_collision;
    parameter_list.one_hard_collision = temp_one_hard_collision;
    
    //causal diffusion or not
    bool temp_causal_diffusion = false;
    tempinput = Util::StringFind4(input_file, "causal_diffusion");
    if (tempinput != "empty")
    ...
    if (tempinput != "empty")
	istringstream(tempinput) >> temp_jet_sigma_x;
    parameter_list.jet_sigma_x = temp_jet_sigma_x;
    
    double temp_jet_sigma_tau = 0.1;
    tempinput = Util::StringFind4(input_file, "jet_sigma_tau");
    if (tempinput != "empty")
	istringstream(tempinput) >> temp_jet_sigma_tau;
    parameter_list.jet_sigma_tau = temp_jet_sigma_tau;
    
    double temp_jet_sigma_eta = 0.2;
    tempinput = Util::StringFind4(input_file, "jet_sigma_eta");
    if (tempinput != "empty")
	istringstream(tempinput) >> temp_jet_sigma_eta;
    parameter_list.jet_sigma_eta = temp_jet_sigma_eta;
    ...
    //DOES NOTHING
    bool temp_introduce_jets_flag = 0;
    tempinput = Util::StringFind4(input_file, "introduce_jets_flag");
    if (tempinput != "empty")
    ...
    tempinput = Util::StringFind4(input_file,
                                  "Initial_Binaries_input_filename");
    if (tempinput != "empty")
        tempbinsName.assign(tempinput);
    parameter_list.binsName.assign(tempbinsName);
...
void check_parameters(InitData &parameter_list, std::string input_file) {
    ...
    if (parameter_list.jet_medium_response_flag < 0 || parameter_list.jet_medium_response_flag > 3) {
        music_message << "jet_medium_response_flag " << parameter_list.jet_medium_response_flag
                      << "not defined";
        music_message.flush("error");
        exit(1);
    }
