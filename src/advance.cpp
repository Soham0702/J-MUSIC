...
void Advance::AdvanceIt(double tau, SCGrid &arena_prev, SCGrid &arena_current,
  ...
  const int grid_ny   = arena_current.nY();
    ...
    int ieta;
    int t_step = ((tau-DATA.tau0)+0.00001)/DATA.delta_tau;	
    if(DATA.jet_medium_response_flag == 1 || DATA.jet_medium_response_flag == 2) {
        if(t_step%DATA.JetSourceEveryNTimestep == 0){
//    	    hydro_source_terms.CountSources();
    	    hydro_source_terms.RemoveSources(tau);
	    cout << " Removing Sources with tau= " << tau  << endl;
...
void Advance::FirstRKStepT(const double tau, double x_local, double y_local,
    ...
    double *jet_mu = new double[4];
    for (int ii = 0; ii < 4; ii++) {
        jet_mu[ii] = 0.0;
    }
    int t_step = ((tau-DATA.tau0)+0.00001)/DATA.delta_tau;	
    if(t_step%DATA.JetSourceEveryNTimestep == 0){
	if(DATA.jet_medium_response_flag == 1 && rk_flag == 0 ) {
//        double *u_local = new double[4];
//        for (int ii = 0; ii < 4; ii++) {
//            u_local[ii] = grid_pt->u[rk_flag][ii];
//        }
		hydro_source_terms.get_jet_energy_source(
			tau_rk, tau_next, x_local, y_local, eta_s_local, jet_mu, arena_current(ix,iy,ieta).epsilon);
		for(int ii = 0; ii < 4; ii++) {
			jet_mu[ii] *= tau_rk;
		}
//		delete[] u_local;
	...
	for (int ii = 0; ii < 4; ii++) {
            qi_source[ii] += jet_mu[ii];
	    //if (jet_mu[0]!=0) cout << setprecision(6) << " Adding jet momentum " << jet_mu[0] << " " << jet_mu[1] << " " << jet_mu[2] << " " << jet_mu[3] << endl;
	}
...
void Advance::QuestRevert(double tau, Cell_small *grid_pt,
    ...
    double temperature=eos.get_temperature(e_local, rhob);
